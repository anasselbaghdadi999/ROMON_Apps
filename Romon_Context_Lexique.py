...
class My_App:
    def __init__(self, master):
        ...
        # مسار مجلد المشروع الأساسي لضمان ثبات القراءة والحفظ
        self.project_path = Path("/home/anass/Documents/Programming_projects/python projects/ROMON_Apps")
    ...
    def search_word_context(self, word_entry, result_listbox, file_name):
        ...
        target_file_path = self.project_path / file_name
    ...
    def save_explanation_to_file(self, romon, word, context, ai_response_text):
        """حفظ الشروحات مباشرة في مجلد المشروع لسهولة الوصول إليها"""
        try:
            # التأكد من إنشاء المجلد في حال لم يكن موجوداً
            if not self.project_path.exists():
                self.project_path.mkdir(parents=True, exist_ok=True)
                
            # تحديد اسم الملف بناءً على الرواية
            if "boîte" in romon.lower() or "boite" in romon.lower():
                output_file_path = self.project_path / "explication_la_boite_a_merveilles.txt"
            elif "antigone" in romon.lower():
                output_file_path = self.project_path / "explication_antigone.txt"
            else:
                output_file_path = self.project_path / "explication_le_dernier_jour_d_un_condamne.txt"

            # استخدام وضع "a" (Append) لضمان عدم مسح البيانات السابقة
            with open(output_file_path, "a", encoding="utf-8") as output_file:
                output_file.write(f"=== {romon} ===\n")
                output_file.write(f"Mot/Expression: {word}\n")
                output_file.write(f"Contexte: {context}\n")
                output_file.write(f"Explication:\n{ai_response_text}\n")
                output_file.write("-" * 50 + "\n\n")

        except Exception as e:
            # إرسال رسالة خطأ صريحة للواجهة
            self.master.after(0, lambda err=e: messagebox.showerror("Erreur de sauvegarde", f"Le fichier n'a pas pu être enregistré :\n{err}"))
    ...
    def fetch_ai_explanation(self, word, context, explanation_box, romon):
        ...
        try:
            ...
            ai_response_text = ar(raw_response) 
            self.update_textbox_content(explanation_box, ai_response_text, self.ans_color)

            # استدعاء دالة الحفظ
            self.save_explanation_to_file(romon, word, context, raw_response)
